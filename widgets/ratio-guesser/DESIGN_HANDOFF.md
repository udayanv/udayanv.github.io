...
Build a zero-dependency static web widget that teaches aspect-ratio estimation. In every round, a person compares a generated reference silhouette with an upright rectangle whose **width is fixed** and whose **height is set by one slider**. On submission, show a visual comparison and numerical feedback.
...
- Upright rectangle, rotated rectangle, plus sign, rotated plus sign, offset plus sign, oval, and rotated oval modes.
...
- Arbitrary smooth blob generation and its selectable mode.
...
The adjustable rectangle is always upright:

`estimateRatio = adjustableHeight / fixedWidth`

Its fixed width can be any convenient rendering value. Its rendered height must be derived directly from the slider's selected ratio.
...
Changing the mode or difficulty before submission immediately generates a fresh, unanswered round. The slider must be disabled after submission; Next shape creates another round using the current selections.
...
- A visible label and native `<select>` for **Shape mode**.
...
1. **Reference panel**
   - Title: “Reference”.
   - Draw only the filled reference silhouette before submission.
...
2. **Your estimate panel**
   - Title: “Your estimate”.
   - Draw the upright, filled adjustable rectangle centered in a similarly sized frame.
   - Keep its width constant through the round. Change only height as the slider moves.
   - Keep the rectangle visibly within its panel for every supported ratio.
...
- draw the true reference silhouette in a solid, high-contrast color;
- draw the user's upright rectangle in a contrasting semi-transparent fill and/or clear outline;
- retain the reference's display rotation in this comparison;
...
Implement each mode as an isolated generator rather than branching through the rendering layer. A generator returns shape metadata and a renderable geometry description.
...
```js
{
  ...
  geometry: { /* mode-specific local geometry */ },
  draw(context, options) { /* draw from local geometry, then optional rotation */ }
}
```
...
- All generators must return a connected, legible filled silhouette.

### Shipped modes

| Mode | Local construction | Constraints |
| --- | --- | --- |
| Upright rectangle | One filled rectangle. | `rotationDegrees = 0`. |
| Rotated rectangle | One filled local rectangle then a display rotation. | Exclude near-upright angles. |
| Plus sign | Union of centered filled vertical and horizontal bars. | Bars have non-zero thickness and overlap. |
| Rotated plus sign | Centered plus sign then display rotation. | Same local measurements as plus sign; exclude near-upright angles. |
| Offset plus sign | Union of intersecting horizontal and vertical bars with an offset intersection. | Keep the intersection inside both bars so the result remains connected. Do not rotate in the first release. |
| Oval | Filled ellipse with local radii derived from local width and height. | `rotationDegrees = 0`. |
| Rotated oval | Filled local ellipse then a display rotation. | Exclude near-upright angles. |
...
Reserve an `arbitrary-blob` generator ID but do not add it to the first-release selector. A future generator must satisfy the same contract and produce a smooth, connected, non-self-intersecting local silhouette with explicit local dimensions. No blob algorithm is approved yet.
...
- all seven shipped modes can be selected and rendered without clipping;
- each round produces a target within `0.4–2.5` based on its local dimensions;
- rotation never changes the target ratio;
- the estimate rectangle has fixed width and changes only height;
...
- mode or difficulty changes create a fresh unanswered round;
